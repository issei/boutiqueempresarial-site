...
| D10 | LDU (`dataProcessingOptions ['LDU']`) ativo para 100% do tráfego | `fbq('dataProcessingOptions', ['LDU'], 1, 1000)` | LDU é mecanismo **CCPA/Califórnia**; ligado globalmente ele restringe o uso dos dados sem ganho de conformidade LGPD | **Parcialmente tratado**: o CAPI passou a enviar o mesmo LDU `(1, 1000)` do Pixel (ver `design/formulario-tracking-funil.md`), para os dois canais tratarem o evento igual. Forçar LDU a todos os visitantes segue como decisão de negócio pendente |
...
> A ordem antiga (com Instagram/Site, Autonomia, Prioridade, Informações Adicionais) diverge deste cabeçalho, então no primeiro `doPost` após o deploy o `ensureSheet()` arquiva a aba `Respostas` como `Respostas_legado_<timestamp>` e cria a nova. Nenhum dado histórico é apagado.
